...
当前登录链路仍是 **demo/mock-first**：

- `src/views/common/LoginView.vue`
  - 只有 3 个“以员工/店长/运营经理身份进入”按钮
  - 调用 `loginAs(role)` → `POST /api/auth/mock-login`
- `src/views/common/AuthCallbackView.vue`
  - 只检查 `?code=` 是否存在
  - 然后直接按 `?role=` 调 `loginAs(role)`
  - 本质仍是 mock 回调，不是真正 OAuth callback 落地
- `src/stores/session.ts`
  - 本地存储 key：`atlas_demo_session`
  - 启动时读取 localStorage，并尝试 `GET /api/auth/me`
  - **如果 `/auth/me` 失败，会保留旧 session 继续跑页面**
- `src/router/index.ts`
...
这对 Sprint 1 demo 很合适，但对 Sprint 2 真登录联调有一个明显风险：
...
现在的 `AuthCallbackView` 只要 URL 里有 `code`，就会继续调用 mock 登录；这会让“回调页 UI 看起来成功”，但实际没有验证：

- code 是否有效
- state 是否匹配
- 企业微信身份是否映射到系统用户
- 后端 session/token 是否真的可用

这个页面是 Sprint 2 **最不能继续假跑** 的地方。

---

## 2.3 Session bootstrap（应用启动鉴权）

目标：从“本地 session 容错优先”改成“真实登录态优先，mock 仅限显式开发模式”。
...
我不建议一上来同时改 login + callback + dashboard + 全部业务页 fallback。那会把排错面拉太大。

### 最稳的首波切片

**切片目标：只打通“真实 WeCom 登录建立 session → 进入首页/待开通页”这条链。**
...
2. `/auth/callback`
   ...
   - 正确写 session
3. `session.ts`
   - 增加真实 session 写入/清理逻辑
   - 收紧 bootstrap 对失效 session 的处理
4. `/pending-access`
   - 承接真实无角色用户
5. `/home`
   - 先不强求 dashboard 真数据，只要求能基于真实 user 信息进入并显示
...
对当前 atlas-web 来说，Sprint 2 真正的首要工作不是“把所有业务页都接真”，而是：

> **先把 mock-first 的登录骨架，收敛成 real-auth-first 的会话骨架。**

最关键的三个点：

1. `LoginView` 不再以 mock 角色按钮为主入口
2. `AuthCallbackView` 不再假装处理 code，而是真正交换 session
3. `bootstrapSession()` 不再默默保留失效本地 session
